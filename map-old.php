...
<html>
    ...
    <body>
        <div id="map"></div>
           <?php                   
               require 'config.php';
               $query= "SELECT cid,title,lat,lng,`data` FROM a_all_items";
            ...
            class Location {
                ...
                function __construct(String $title, String $lat, String $lng, String $cid) {
                    $this->title = $title;
                    $this->lat = $lat;
                    $this->lng = $lng;
                    $this->cid = $cid;
            ...
            while($vals=mysqli_fetch_object($items)){
                ...
                array_push($r_vals, new Location($vals->title, $vals->lat,$vals->lng, $vals->cid));
            }
                ?>
		...
		const build_content_popup = (title,data, id) => {
			...
			content += `</table><br><a href=${hrefFix_to(id)}>Read More</a>`
			return content
		}
                ...
                const mapInfoToMarker = (title="test", data="test", coords, id) => {
                    ...
                    testMarker.addListener("click", () => {
                    	...
                    	infoWindow = new google.maps.InfoWindow({
                        	content: build_content_popup(title, data, id) 
                        })
                ...
                marker.forEach((mark) => {
                    if (mark.coords.lat != 0 && mark.coords.lng != 0) {
                        mapInfoToMarker(mark.title, mark.data, mark.coords, mark.cid);
                    }
                })
